		...
		<div class="row mb-2">
			...
			<div class="col-sm-6">
				<ol class="breadcrumb float-sm-right">
					...
					<li class="breadcrumb-item"><a href="{{site_url('user/profil')}}">Profil</a></li>
...
<script>
	$(document).ready(function () {
		...
		$('#form-ubah').submit('click', function (event) {
			...
			$.ajax({
				method: "POST",
				url: "{{base_url('user/profil/aksi_ubah')}}",
				data: formData,
				cache: false,
				contentType: false,
				processData: false,
				...
				success: function (data) {
					...
					if (data.error) {
						...
						if (data.password_error != '') {
							...
							$('#password-msg').html(data.password_error);
						} else {
						...
						setTimeout(function () {
							...
							"{{base_url('user/profil/ubah?id_user='.$id_user)}}";
						}, 1000);
					}
				},
				error: function (data) {
